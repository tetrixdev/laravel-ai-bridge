...
Measured against Claude Code 2.1.x, the bridge's longest gap between frames on a turn that ran a 25-second shell command was **5.2 seconds** — the CLI keeps talking through a long tool, so ordinary slow work does not resemble silence. Real silence means the CLI has stopped speaking altogether, which is the signal worth acting on. The old 300-second request timeout killed exactly that turn, punctually, mid-work — and punctuality is the tell, because a crash is never that precise. The silence clock resets on **every** frame the adapter emits: delta, tool call, tool result. `request_timeout` remains as a backstop for a server that wants a hard ceiling.
